...
public class CommandCharacter extends Command {
	...
	public void execute(Message message, String... args) {
		...
		Map<Stage,Float> stageFloatMap = new HashMap<>();
		...
		for(Stage s : Stage.values()) {
			...
			stageFloatMap.put(s, (float) swon / (swon + slose));
		}
		...
		b.addField("Best Stages",
				stages.get(stages.size() - 1).readable_name + " (" + stageFloatMap.get(stages.get(stages.size() - 1)) + "%)\n" +
						stages.get(stages.size() - 2).readable_name + " (" + stageFloatMap.get(stages.get(stages.size() - 2)) + "%)\n" +
						stages.get(stages.size() - 3).readable_name + " (" + stageFloatMap.get(stages.get(stages.size() - 3)) + "%)", false);

		b.addField("Worst Stages",
				stages.get(0).readable_name + " (" + stageFloatMap.get(stages.get(0)) + "%)\n" +
						stages.get(1).readable_name + " (" + stageFloatMap.get(stages.get(1)) + "%)\n" +
						stages.get(2).readable_name + " (" + stageFloatMap.get(stages.get(2)) + "%)", true);
